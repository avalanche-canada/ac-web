...
var forecasts = polygons.features.map(p => {

    var centroid = centroids.features.find(c => { 
      return c.properties.id === p.properties.id 
    });
    var fxData = data.find(d => d.id === p.properties.id );

    p.id = fxData.id;
    Object.assign(p.properties, fxData);

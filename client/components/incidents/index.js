...
export const IncidentList = ({status, error, data, page, filters, onPageChange, onFilterChange})  => {
    return <div>
        <Status isLoading={status===PENDING} isLoaded={status===FULFILLED} isError={status===ERROR}/>
        { status === FULFILLED
            ? <IncidentTable data={data} page={page} filters={filters} onPageChange={onPageChange} onFilterChange={onFilterChange} />
            : <div></div> }
    </div>
};
...
class IncidentTable extends PureComponent {
    ...
    render() {
        ...
        return <div>
            ...
            <Pagination total={page_count} active={this.props.page} onChange={this.props.onPageChange} />
        </div>;
    }
}
...
class IncidentFilters extends PureComponent {
    handleChangeFrom = fromSeason => this.props.onChange({from: fromSeason, to: this.props.values.to })
    handleChangeTo   = toSeason => this.props.onChange({from: this.props.values.from, to: toSeason })
    render() {
        ...
        const froms = new Map(f.filter(f => startsBefore(f, this.props.values.to)).map(f => [f.season, f.title]))
        const tos = new Map(FILTERS.filter(f => startsAfter(f, this.props.values.from)).map(f => [f.season, f.title]))
        return <div>
...
const Row = ({inc}) => {
    return <t.Row>
        ...
        <t.Cell><a href={`/incidents/${inc.id}`}>view</a></t.Cell>
    </t.Row>;
}
...
const IncPage = ({incident}) => {
     return <div>
        ...
        <IncWx weather={incident.weather_obs} weatherComment={incident.weather_comment} />
        <IncSnow snowpack={incident.snowpack_obs} snowpackComment={incident.snowpack_comment} />
...
const IncSummary = ({incident}) => {
    return <div>
        ...
        <table className={styles.SummaryTable}>
            ...
            <tr>
                <th>Date</th>
                <td><Time format="YYYY-MM-DD" value={incident.ob_date} /></td>
            </tr>
            <SummaryVal name="Location"             val={incident.location} />
            <SummaryVal name="Location Description" val={incident.location_desc} />
            <SummaryVal name="Province"             val={incident.location_province} />
            {/* 
                TODO(wnh): Figure out where this comes from. Needs work in the API
                <SummaryVal name="Mountain Range"       val="TODO" />
            */}
            <SummaryVal name="Coordinates"          val={incident.location_coords} />
            <SummaryVal name="Elevation"            val={incident.location_elevation} suffix="m" />
            <SummaryVal name="Activity"             val={incident.group_activity} />
            <SummaryVal name="Involvement"          val={incident.num_involved} />
...
const RowVal = ({val, suffix}) => {
    if(val) {
        return <t.Cell>{val}{suffix}</t.Cell>
    } else {
...
const IncAvalanche = ({avalanches}) => {
    const rows = avalanches.map((av, i) => <t.Row key={i}>
        <t.Cell><Time format="YYYY-MM-DD HH:mm" value={av.observation_date} /></t.Cell>
        <RowVal val={av.size} />
        <RowVal val={av.type} />
    ...
    return <div>
        ...
        <t.Table>
            <t.Header>
                ...
                <t.HeaderCell>Precipitation Type & Intensity</t.HeaderCell>
                </t.Row>
            </t.Header>
            <t.TBody>
                <t.Row>
        ...
        { weatherComment
            ? <p><b>Weather Comment:</b> {weatherComment}</p>
            : <p />
        }
    </div>
}
...
const IncSnow = ({snowpack, snowpackComment}) => {
    ...
    return <div>
        ...
        { snowpackComment
            ? <p><b>Snowpack Comment:</b> {snowpackComment}</p>
            : <p />
        }
    </div>
}
...
const IncDocuments = ({docs}) => {
    console.log( docs)
    const rows = docs.map((d, i) => <t.Row key={i}>

...
export class Chart extends PureComponent {
    ...
    get indicator() {
        ...
        const x = X_COORDINATES[terrain - 1]
        const y = Y_COORDINATES[danger - 1]
    ...
    render() {
        return (
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 225 173">
                ...
                />
                {this.indicator}
                {X_COORDINATES.map((x, index) => (
                    <text
                        x={x}
                        y={175}
                        textAnchor="middle"
                        fontSize={8}
                        fill={TERRAIN_RATING_COLOR}>
                        {X_LABELS[index]}
                    </text>
                ))}
                {Y_COORDINATES.map((y, index) => (
                    <text
                        x={10}
                        y={y}
                        textAnchor="left"
                        fill={DANGER_RATING_COLOR}
                        fontSize={8}>
                        {Y_LABELS[index]}
                    </text>
                ))}
...
const X_COORDINATES = [42.3, 111.9, 181.3]
const Y_COORDINATES = [149.6, 117.7, 85.9, 53.9, 25.7]
const X_LABELS = [
    TerrainRatingTexts.get(SIMPLE),
    TerrainRatingTexts.get(CHALLENGING),
    TerrainRatingTexts.get(COMPLEX),
]
const Y_LABELS = [
    ForecastRatingTexts.get(LOW),
    ForecastRatingTexts.get(MODERATE),
    ForecastRatingTexts.get(CONSIDERABLE),
    ForecastRatingTexts.get(HIGH),
    ForecastRatingTexts.get(EXTREME),
]
const LABEL_FONT_SIZE = 8
const DANGER_RATING_COLOR = '#245EAC'
const TERRAIN_RATING_COLOR = '#245EAC'

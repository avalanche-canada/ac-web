export const AvalancheCanada = {
    id: '57d9c088-1871-11e6-b6ba-3e1d05defe78',
    children: [{
        id: '57d9c4d4-1871-11e6-b6ba-3e1d05defe78',
        label: 'Mountain Information Network',
        children: [{
            id: '57d9c722-1871-11e6-b6ba-3e1d05defe78',
            label: 'Overview',
            headline: 'Explore the MINs',
            to: '/min',
        }, {
            id: '57d9c966-1871-11e6-b6ba-3e1d05defe78',
            label: 'Create Report',
            headline: 'Create your own min report',
            to: '/min/submit',
        }]
    }, {
        id: '57d9cbdc-1871-11e6-b6ba-3e1d05defe78',
        label: 'Avalanche Information',
        children: [{
            id: '57d9ce20-1871-11e6-b6ba-3e1d05defe78',
            label: 'Forecasts',
            header: true,
            to: '/map/forecasts',
            children: []
        }, {
            id: '57d9d3a2-1871-11e6-b6ba-3e1d05defe78',
            label: 'Blogs',
            header: true,
            to: '/blogs'
        }, {
            id: '57d9d596-1871-11e6-b6ba-3e1d05defe78',
            label: 'Historic Incidents',
            header: true,
            to: '/historic-incidents'
        }, {
            id: '57d9d9c4-1871-11e6-b6ba-3e1d05defe78',
            label: 'Mountain Weather Forecast',
            header: true,
            to: '/weather'
        }, {
            id: '57d9dbc2-1871-11e6-b6ba-3e1d05defe78',
            label: 'Trip Planner',
            header: true,
            to: '/trip-planner'
        }]
    }, {
        id: '57d9de74-1871-11e6-b6ba-3e1d05defe78',
        label: 'Learn',
        children: [{
            id: '57d9e090-1871-11e6-b6ba-3e1d05defe78',
            label: 'Avalanche Canada Training Courses',
            header: true,
            to: '/training',
            children: [{
                id: '57d9e2de-1871-11e6-b6ba-3e1d05defe78',
                label: 'Overview',
                to: '/training/overview',
            }, {
                id: '57d9e572-1871-11e6-b6ba-3e1d05defe78',
                label: 'Online Avalanche Tutorial',
                to: '/training/online-primer',
            }, {
                id: '57d9e98c-1871-11e6-b6ba-3e1d05defe78',
                label: 'Cours d’avalanche en ligne (français)',
                to: '/training/online-french',
            }, {
                id: '57daca82-1871-11e6-b6ba-3e1d05defe78',
                label: 'Avalanche Skills Training 1',
                to: '/training/ast/1',
            }, {
                id: '57dace38-1871-11e6-b6ba-3e1d05defe78',
                label: 'Avalanche Skills Training 2',
                to: '/training/ast/2',
            }, {
                id: '57dacfaa-1871-11e6-b6ba-3e1d05defe78',
                label: 'Companion Rescue Skills',
                to: '/training/crs',
            }, {
                id: '57dad0d6-1871-11e6-b6ba-3e1d05defe78',
                label: 'Find a course',
                to: '/training/ast/courses',
            }, {
                id: '57dad48c-1871-11e6-b6ba-3e1d05defe78',
                label: 'Find a provider',
                to: '/training/ast/providers',
            }, {
                id: '57dad5c2-1871-11e6-b6ba-3e1d05defe78',
                label: 'Essential Gear',
                to: '/gear',
            }],
        }, {
            id: '57dad6da-1871-11e6-b6ba-3e1d05defe78',
            label: 'Snowmobile',
            header: true,
            to: '/sled',
            children: [{
                id: '57dad7f2-1871-11e6-b6ba-3e1d05defe78',
                label: 'Throttle Decisions',
                to: '/sled/video-featured',
            }, {
                id: '57dadaf4-1871-11e6-b6ba-3e1d05defe78',
                label: 'Mountain Information Network',
                to: '/sled/min',
            }, {
                id: '57dadc0c-1871-11e6-b6ba-3e1d05defe78',
                label: 'Al Hodgson Memorial Fund',
                to: '/foundation/programs/memorial-funds',
            }, {
                id: '57dadd1a-1871-11e6-b6ba-3e1d05defe78',
                label: 'Essential gear',
                to: '/gear',
            }],
        }, {
            id: '57dade28-1871-11e6-b6ba-3e1d05defe78',
            label: 'Youth',
            header: true,
            to: '/youth',
            children: [{
                id: '57dadf5e-1871-11e6-b6ba-3e1d05defe78',
                label: 'Overview',
                to: '/youth/overview',
            }, {
                id: '57dae2d8-1871-11e6-b6ba-3e1d05defe78',
                label: 'Programs',
                to: '/youth/programs',
            }, {
                id: '57dae3fa-1871-11e6-b6ba-3e1d05defe78',
                label: 'Resources',
                to: '/youth/resources',
            }, {
                id: '57dae4ea-1871-11e6-b6ba-3e1d05defe78',
                label: 'Curriculum',
                to: '/youth/curriculum',
            }],
        }, {
            id: '57dae5e4-1871-11e6-b6ba-3e1d05defe78',
            label: 'Ambassadors',
            header: true,
            to: '/ambassadors',
            children: [{
                id: '57dae6de-1871-11e6-b6ba-3e1d05defe78',
                label: 'Overview',
                to: '/ambassadors',
            }, {
                id: '57daea58-1871-11e6-b6ba-3e1d05defe78',
                label: 'Nadine Overwater',
                to: '/ambassadors/nadine-overwater',
            }, {
                id: '57daeb98-1871-11e6-b6ba-3e1d05defe78',
                label: 'Robin van Gyn',
                to: '/ambassadors/robin-van-gyn',
            }],
        }]
    }, {
        id: '57daec92-1871-11e6-b6ba-3e1d05defe78',
        label: 'About Us',
        noWrap: true,
        children: [{
            id: '57daed96-1871-11e6-b6ba-3e1d05defe78',
            label: 'About',
            header: true,
            to: '/about',
            children: [{
                id: '57daee86-1871-11e6-b6ba-3e1d05defe78',
                label: 'Vison',
                to: '/about/vison',
                children: []
            }, {
                id: '57daef80-1871-11e6-b6ba-3e1d05defe78',
                label: 'Board',
                to: '/about/board',
                children: []
            }, {
                id: '57daf368-1871-11e6-b6ba-3e1d05defe78',
                label: 'Staff',
                to: '/about/staff',
                children: []
            }, {
                id: '57daf4da-1871-11e6-b6ba-3e1d05defe78',
                label: 'Awards',
                to: '/about/awards',
                children: []
            }, {
                id: '57daf5e8-1871-11e6-b6ba-3e1d05defe78',
                label: 'Foundation',
                to: '/foundation',
                children: []
            }]
        }, {
            id: '57daf6e2-1871-11e6-b6ba-3e1d05defe78',
            label: 'Membership',
            header: true,
            to: '/membership',
            children: [{
                id: '57daf7d2-1871-11e6-b6ba-3e1d05defe78',
                to: 'https://membership.avalanche.ca/np/clients/cac/membershipJoin.jsp',
                label: 'Individual',
            }, {
                id: '57daf8d6-1871-11e6-b6ba-3e1d05defe78',
                to: 'https://membership.avalanche.ca/np/clients/cac/membershipJoin.jsp?&constTypeFlag=org',
                label: 'Organization',
            }, {
                id: '57daf9d0-1871-11e6-b6ba-3e1d05defe78',
                to: 'https://membership.avalanche.ca/np/clients/cac/login.jsp',
                label: 'Already A Member?',
            }]
        }, {
            id: '57dafd54-1871-11e6-b6ba-3e1d05defe78',
            label: 'Sponsors',
            header: true,
            to: '/sponsors',
            children: [{
                id: '57db00ba-1871-11e6-b6ba-3e1d05defe78',
                label: 'Program Partners',
                to:'sponsors/partner'
            }, {
                id: '57db0204-1871-11e6-b6ba-3e1d05defe78',
                label: 'Premier Sponsors',
                to:'sponsors/funding'
            }, {
                id: '57db033a-1871-11e6-b6ba-3e1d05defe78',
                label: 'Supporters',
                to:'sponsors/supplier'
            }, {
                id: '57db043e-1871-11e6-b6ba-3e1d05defe78',
                label: 'Contributors',
                to:'sponsors/associate'
            }]
        }, {
            id: '57db0538-1871-11e6-b6ba-3e1d05defe78',
            label: 'Collaborators',
            header: true,
            to: '/collaborators',
            children: [{
                id: '57db063c-1871-11e6-b6ba-3e1d05defe78',
                label: 'Government Partners',
                to:'collaborators/government'
            }, {
                id: '57db0aba-1871-11e6-b6ba-3e1d05defe78',
                label: 'Contribution Programs',
                to:'collaborators/contribution'
            }, {
                id: '57db0be6-1871-11e6-b6ba-3e1d05defe78',
                label: 'Other Agencies and Organisation',
                to:'collaborators/other'
            }]
        }]
    }, {
        id: '57db0cf4-1871-11e6-b6ba-3e1d05defe78',
        label: 'News & Events',
        children: [{
            id: '57db0dee-1871-11e6-b6ba-3e1d05defe78',
            to: '/news',
            label: 'News',
            headline: 'Some content for the News.',
        }, {
            id: '57db0ef2-1871-11e6-b6ba-3e1d05defe78',
            to: '/events',
            label: 'Events',
            headline: 'Some content for the Events.',
        }, {
            id: '57db0fe2-1871-11e6-b6ba-3e1d05defe78',
            to: '/outreach',
            label: 'Outreach',
            headline: 'Some content for the Outreach.',
        }]
    }, {
        id: '57db10e6-1871-11e6-b6ba-3e1d05defe78',
        label: 'Store',
        children: [{
            id: '57db1500-1871-11e6-b6ba-3e1d05defe78',
            to: '/auctions',
            label: 'Web Auction',
            headline: 'Access the Web auction.',
        }]
    }]
}
export const AvalancheCanadaFoundation = {
    id: 'f002466e-1b91-11e6-b6ba-3e1d05defe78',
    children: [{
        id: 'f0024aba-1b91-11e6-b6ba-3e1d05defe78',
        label: 'About',
        children: [{
            id: 'f0024d12-1b91-11e6-b6ba-3e1d05defe78',
            label: 'Mission',
            to: '/foundation/mission',
            header: true,
        }, {
            id: 'f00251a4-1b91-11e6-b6ba-3e1d05defe78',
            label: 'Annual Reports and Financial Statements',
            to: '/foundation/about/reports',
            header: true,
        }, {
            id: 'f002535c-1b91-11e6-b6ba-3e1d05defe78',
            label: 'Board Of Directors',
            to: '/foundation/about/board',
            header: true,
        }, {
            id: 'f00254d8-1b91-11e6-b6ba-3e1d05defe78',
            label: 'Honourary Directors',
            to: '/foundation/about/honourary',
            header: true,
        }, {
            id: 'f0025640-1b91-11e6-b6ba-3e1d05defe78',
            label: 'Contact',
            to: '/foundation/about/contact',
            header: true,
        }, {
            id: 'f00257b2-1b91-11e6-b6ba-3e1d05defe78',
            label: 'Subscribe to newsletter',
            to: '/foundation/about/subscribe',
            header: true,
        }]
    }, {
        id: 'f002591a-1b91-11e6-b6ba-3e1d05defe78',
        label: 'Programs',
        children: [{
            id: 'f0025a8c-1b91-11e6-b6ba-3e1d05defe78',
            label: 'Avalanche Canada',
            to: '/foundation/programs/ac',
            headline: 'Headline for Avalanche Canada',
        }, {
            id: 'f0025f82-1b91-11e6-b6ba-3e1d05defe78',
            label: 'Memorial Funds and Scholarships',
            to: '/foundation/programs/memorial-funds',
            headline: 'Headline for Memorial Funds and Scholarships',
        }, {
            id: 'f00260fe-1b91-11e6-b6ba-3e1d05defe78',
            label: 'Memorial Donations',
            to: '/foundation/programs/memorial-donations',
            headline: 'Headline for Memorial Donations',
        }]
    }, {
        id: 'f002625c-1b91-11e6-b6ba-3e1d05defe78',
        label: 'Contributors',
        children: [{
            id: 'f00263ce-1b91-11e6-b6ba-3e1d05defe78',
            label: 'Donors',
            to: '/foundation/donors',
            headline: 'Headline for Donors',
        }, {
            id: 'f0026540-1b91-11e6-b6ba-3e1d05defe78',
            label: 'Event Sponsors',
            to: '/foundation/eventsponsor',
            headline: 'Headline for Event Sponsors',
        }]
    }, {
        id: 'f00266a8-1b91-11e6-b6ba-3e1d05defe78',
        label: 'News & Events',
        children: [{
            id: 'f0026a90-1b91-11e6-b6ba-3e1d05defe78',
            label: 'News',
            to: '/foundation/news',
            headline: 'Headline for News',
        }, {
            id: 'f0026c20-1b91-11e6-b6ba-3e1d05defe78',
            label: 'Events',
            to: '/foundation/events',
            headline: 'Headline for Events',
        }]
    }]
}

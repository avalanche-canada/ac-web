...
angular.module('avalancheCanadaApp')
  ...
  .controller('MoreCtrl', function ($scope, $rootScope, $location, Prismic, $log) {
    ...
    Prismic.ctx().then(function(ctx){
        ...
        ctx.api.form('everything').query('[[:d = at(document.type, "news")] [:d = at(document.tags, ["featured"])]]').ref(ctx.ref).submit(function(err, documents){
            if (err) {
                $log.error('error getting featured news events from prismic');
            }
            else {
                $scope.news_featured = documents.results;
            }
        });
